    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 36, flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p style={{ color: 'var(--text)', fontSize: 14 }}>Welcome back, {user?.name?.split(' ')[0]}</p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {!canCreate && (
            <div style={{ background: 'var(--accent-bg)', border: '1px solid var(--accent-border)', borderRadius: 'var(--radius-sm)', padding: '8px 14px', fontSize: 13, color: 'var(--accent2)' }}>
      ...
      <div className="card" style={{ marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, padding: '16px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span className={`badge ${user?.plan === 'premium' ? 'badge-purple' : 'badge-gray'}`}>
            {user?.plan === 'premium' ? 'Premium' : 'Starter'}
          </span>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '80px 24px' }}>
          <div style={{ width: 64, height: 64, background: 'var(--accent-bg)', border: '1px solid var(--accent-border)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 28, color: 'var(--accent)' }}>+</div>
          <h2 style={{ fontSize: 22, fontWeight: 600, marginBottom: 8 }}>No websites yet</h2>
          <p style={{ color: 'var(--text)', marginBottom: 28, fontSize: 15 }}>Create your first business website and get it live today.</p>
          <Link to="/studio/new" className="btn btn-primary btn-lg">Build Your First Site →</Link>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
          {sites.map(site => (
            <div key={site.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 0, padding: 0, overflow: 'hidden' }}>
              {/* Preview header */}
              <div style={{ background: 'linear-gradient(135deg, var(--accent-bg), var(--bg3))', padding: '24px 20px', borderBottom: '1px solid var(--border)', position: 'relative' }}>
                <div style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--accent)', marginBottom: 12 }}>{site.category}</div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-h)', marginBottom: 4 }}>{site.business_name}</h3>
                <div style={{ fontSize: 12, color: 'var(--accent2)', fontFamily: 'var(--mono)' }}>{site.subdomain}.businessstudios.com</div>
                <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', gap: 6 }}>
                  <span className="badge badge-green">● Live</span>
                  <span className="badge badge-gray">{site.category}</span>
                </div>
              </div>
              {/* Info */}
              <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'var(--text)' }}>
                  <span>Layout: <strong style={{ color: 'var(--text-m)' }}>{site.layout_style?.replace('_', ' ')}</strong></span>
              ...
              <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
                <button onClick={() => navigate(`/studio/${site.id}`)} className="btn btn-secondary btn-sm" style={{ flex: 1, justifyContent: 'center' }}>Edit</button>
          ...
          {canCreate && (
            <Link to="/studio/new" style={{ textDecoration: 'none' }}>
              <div style={{ border: '2px dashed var(--border)', borderRadius: 'var(--radius)', padding: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, cursor: 'pointer', minHeight: 200, transition: 'all 0.2s' }}
                onMouseOver={e => { e.currentTarget.style.borderColor = 'var(--accent-border)'; e.currentTarget.style.background = 'var(--accent-bg)'; }}
                onMouseOut={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'transparent'; }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--accent-bg)', border: '1px solid var(--accent-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, color: 'var(--accent2)' }}>+</div>
                <span style={{ color: 'var(--text-m)', fontWeight: 500, fontSize: 15 }}>Create new site</span>
              </div>
            </Link>
          )}
        </div>
