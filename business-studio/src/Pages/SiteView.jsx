...
export default function SiteView() {
  ...
  if (error || !site) return (
    <div style={{ textAlign: 'center', padding: '80px 24px' }}>
      ...
      <p style={{ color: 'var(--text)', marginBottom: 28 }}>The site "{subdomain}" doesn't exist or isn't published.</p>
  ...
  return (
    <div style={{ background: themeVars.bg, minHeight: '100vh', color: themeVars.text }}>
      {/* Top bar - "Powered by BusinessStudio" */}
      <div style={{ background: '#7c3aed', color: '#fff', textAlign: 'center', padding: '8px', fontSize: 13 }}>
        This website was built with{' '}
        <Link to="/" style={{ color: '#fff', fontWeight: 700 }}>BusinessStudio.com</Link>
        {' '} — <Link to="/signup" style={{ color: '#fff', fontWeight: 700 }}>Build yours</Link>
      </div>
      ...
      <header style={{ background: themeVars.bg2, borderBottom: `1px solid ${themeVars.border}`, padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: themeVars.accent }}>{site.category}</div>
        </div>
  ...
  if (comp.type === 'gallery') return (
    <div>
      ...
      {comp.images?.length > 0 ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', background: theme.bg2, borderRadius: 12, color: theme.text }}>Gallery section</div>
      )}
    </div>
  );
  ...
  if (comp.type === 'menu') return (
    <div>
      ...
      {comp.items?.length > 0 ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', background: theme.bg2, borderRadius: 12, color: theme.text }}>Menu items</div>
      )}
    </div>
  );
